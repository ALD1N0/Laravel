...
use App\Models\TwebPendudukRejo;
...
class RejunoController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        try {
            $query = TwebPendudukRejo::query()->where('status_dasar', 1);
    ...
    public function store(Request $request): JsonResponse
    {
        try {
            ...
            $penduduk = TwebPendudukRejo::create($validated);
    ...
    public function update(Request $request, TwebPendudukRejo $TwebPendudukRejo): JsonResponse
    {
        try {
            $validated = $request->validate([
                'nama' => 'sometimes|required|string|max:100',
                'nik' => 'sometimes|required|string|unique:tweb_penduduk,nik,' . $TwebPendudukRejo->id . '|size:16',
                ...
                'email' => 'nullable|email|unique:tweb_penduduk,email,' . $TwebPendudukRejo->id,
                'telepon' => 'nullable|string|max:20',
            ]);

            $TwebPendudukRejo->update($validated);

            return response()->json([
                'status' => 'success',
                'message' => 'Data penduduk berhasil diperbarui',
                'data' => $TwebPendudukRejo->fresh()
            ], 200);
        } catch (ValidationException $e) {
            return response()->json([
                'status' => 'error',
    ...
    public function destroy(TwebPendudukRejo $TwebPendudukRejo): JsonResponse
    {
        try {
            $TwebPendudukRejo->delete();
